...
```bash
node {{ORCHESTRATOR_ROOT}}/orchestrator.mjs status {{AGENT_NAME}}
```

List all agents managed by the orchestrator:
```bash
node {{ORCHESTRATOR_ROOT}}/orchestrator.mjs list
```

Agent overview (all agents, costs, evals, running status):
```bash
node {{ORCHESTRATOR_ROOT}}/orchestrator.mjs dashboard
```

Check pipeline run status:
```bash
node {{ORCHESTRATOR_ROOT}}/orchestrator.mjs pipeline status <run-id>
```

List all pipeline runs:
```bash
node {{ORCHESTRATOR_ROOT}}/orchestrator.mjs pipeline list
```
